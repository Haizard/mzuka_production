...
function buildJitsiIframeUrl(roomId: string, jwt: string | null) {
  const baseUrl = getJitsiIframeUrl(roomId, "#config.startWithAudioMuted=false&config.prejoinPageEnabled=false");
...
export default function ClientMeetingsPage() {
  ...
  const now = new Date();
  const upcoming = meetings.filter(m => !m.endsAt || new Date(m.endsAt) >= now);
  const past = meetings.filter(m => m.endsAt && new Date(m.endsAt) < now);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
      <div>
        ...
        <p className="text-xs text-zinc-500 mt-0.5">Join virtual meetings with the Muzuka Gilbert team</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-[var(--surface)] border border-white/10 rounded-2xl flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-xs text-zinc-600">Your admin team will invite you to virtual meetings here.</p>
        </div>
      ) : (
        <>
          {upcoming.length > 0 && (
            <div className="space-y-3">
              <h2 className="text-sm font-semibold text-zinc-400 uppercase tracking-wide">Upcoming</h2>
                      ...
                      <div className="space-y-1">
                        ...
                        {m.endsAt && (
                          <div className="flex items-center gap-1.5 text-xs text-zinc-500">
                            <Clock className="h-3.5 w-3.5" /><span>Until {fmt(m.endsAt)}</span>
                          </div>
                        )}
                        <div className="flex items-center gap-1.5 text-xs text-zinc-500">
                          <Users className="h-3.5 w-3.5" /><span>Hosted by {m.createdBy.name}</span>
                        </div>
                      </div>
                      <button
                        ...
                        className={`w-full py-2 rounded-xl text-sm font-semibold transition ${
                          isActive ? "bg-red-500/20 text-red-400 border border-red-500/30" : "bg-[var(--gold)] text-black hover:opacity-90"
                        }`}
                      >
                        {isActive ? "Leave Meeting" : "Join Meeting"}
                      </button>
                    </div>
                  );
                })}
          ...
          {past.length > 0 && (
            <div className="space-y-3">
              <h2 className="text-sm font-semibold text-zinc-600 uppercase tracking-wide">Past Meetings</h2>
              <div className="grid gap-3 sm:grid-cols-2">
                {past.map(m => (
                  <div key={m.id} className="bg-[var(--surface)] border border-white/5 rounded-2xl p-4 opacity-60">
                    <h3 className="text-sm font-medium text-zinc-400">{m.title}</h3>
                    <p className="text-xs text-zinc-600 mt-1">{fmt(m.scheduledAt)}</p>
                  </div>
                ))}
              </div>
            </div>
