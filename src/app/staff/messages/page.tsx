  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      <div className="flex items-center gap-2 mb-6">
        ...
        <span className="text-xs text-zinc-500 ml-1">— Private chats with the admin team</span>
      </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {convos.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-32 gap-2 px-4 text-center">
                ...
                <p className="text-xs text-zinc-600">No messages yet from the admin team.</p>
              </div>
            ) : convos.map(c => {
              const other = c.admin;
              ...
              return (
                <button key={c.id} onClick={() => setSelected(c)}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-zinc-500 capitalize">{(other.staffRole ?? other.role).toLowerCase().replace(/_/g," ")}</p>
          ...
          {!selected ? (
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {msgs.length === 0 && (
                  <p className="text-center text-xs text-zinc-600 mt-8">No messages yet.</p>
                )}
                {msgs.map(m => {
